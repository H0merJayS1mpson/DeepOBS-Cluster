...
class SlurmobsRunner():
    ...
    def generate_configurations(self):
        ...
        for key in self.d:
            if 'sbatch' in key:
                self.SBATCH_PARAMS[key] = self.d.get(key)
                
                
        for key in self.SBATCH_PARAMS:
            del self.d[key]
        
        if 'output' in self.d:
            self.SBATCH_PARAMS['output'] = self.d.pop('output')
        
        if 'error' in self.d:
            self.SBATCH_PARAMS['error'] = self.d.pop('error')
